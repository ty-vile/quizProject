...
const DashboardCard: React.FC<Props> = ({ title, description }) => {
  ...
  return (
    <div
      ...
      onClick={() => handleClick()}
    >
      <span className="absolute top-10 z-0 h-20 w-20 rounded-full bg-primary transition-all duration-300 group-hover:scale-[10] lg:group-hover:scale-[15]"></span>
      <div className="relative z-10 mx-auto">
        ...
        <div className="space-y-6 pt-5 text-base leading-7   group-hover:text-white/90 font-josefin">
          <p>{description}</p>
        </div>
      </div>
    </div>
